...
function Shell(me, smart) {
  ...
  me.group = "item"; // Note: shells are items, but can be affected by monster speed
  // Store base speed - actual speed will be calculated when added to scene
  me.baseSpeed = unitsizet2;
  ...
  me.death = killFlip;
  me.spawntype = Koopa;
  // Apply speed multiplier when actually added to the scene
  me.onadding = function () {
    var speedMultiplier = (typeof window.monster_speed_change !== 'undefined')
      ? window.monster_speed_change : 1.0;
    me.speed = me.baseSpeed * speedMultiplier;
  };
...
function Goomba(me) {
  me.width = me.height = 8;
  // Store base speed - actual speed will be calculated when added to scene
  me.baseSpeed = unitsize * .21;
  me.speed = me.baseSpeed; // Will be overridden in onadding if multiplier exists
  me.toly = unitsize;
  ...
  me.onadding = function () {
    var speedMultiplier = (typeof window.monster_speed_change !== 'undefined')
      ? window.monster_speed_change : 1.0;
    me.speed = me.baseSpeed * speedMultiplier;
    console.log('[Goomba added] speed multiplier:', speedMultiplier, 'final speed:', me.speed);
  };
...
function Koopa(me, smart, fly) {
  ...
  // Store base speed - actual speed will be calculated when added to scene
  me.baseSpeed = unitsize * .21;
  me.speed = me.xvel = me.baseSpeed; // Will be overridden in onadding if multiplier exists
  ...
  me.onadding = function () {
    var speedMultiplier = (typeof window.monster_speed_change !== 'undefined')
      ? window.monster_speed_change : 1.0;
    me.speed = me.xvel = me.baseSpeed * speedMultiplier;
  };
  setCharacter(me, name);
...
function HammerBro(me) {
  ...
  me.group = "enemy";
  // Store base speed - actual speed will be calculated when added to scene
  me.baseSpeed = unitsized2; // HammerBro usually moves at unitsized2
  me.speed = me.baseSpeed;
  me.collide = collideEnemy;
...
function BulletBill(me) {
  ...
  // Store base speed - actual speed will be calculated when added to scene
  me.baseSpeed = unitsized2;
  me.speed = me.xvel = me.baseSpeed;
  me.movement = moveSimple;
  me.collide = collideEnemy;
  me.death = killFlip;
  // Apply speed multiplier when actually added to the scene
...
function Blooper(me) {
  ...
  me.onadding = function () {
    var speedMultiplier = (typeof window.monster_speed_change !== 'undefined')
      ? window.monster_speed_change : 1.0;
    me.speed = me.baseSpeed * speedMultiplier;
    // Blooper xvel is complex, but we can scale it?
    // me.xvel is initialized to -unitsized4.
    // moveBlooper modifies xvel/yvel.
    // We should probably scale the movement parameters in moveBlooper?
    // Or just scale speed.
  };
...
function CheepCheep(me, red, jumping) {
  ...
  me.onadding = function () {
    var speedMultiplier = (typeof window.monster_speed_change !== 'undefined')
      ? window.monster_speed_change : 1.0;
    // Scale velocities
    if (me.baseXvel) me.xvel = me.baseXvel * speedMultiplier;
    if (me.baseYvel) me.yvel = me.baseYvel * speedMultiplier;
    // Also set speed property for consistency
    me.speed = Math.abs(me.xvel);
    me.baseSpeed = Math.abs(me.baseXvel);
  };
  setCharacter(me, name);
  addSpriteCycle(me, ["one", "two"]);
}
...
function Lakitu(me, norepeat) {
  ...
  me.group = "enemy";
  // Store base speed - actual speed will be calculated when added to scene
  me.baseSpeed = 0; // Lakitu speed is dynamic based on Mario, but we can scale it?
  // Lakitu movement is complex, usually follows Mario.
  // But we can add onadding just in case we want to scale its movement parameters.
  me.collide = collideEnemy;
